...
`math`_ module can also be evaluated directly.
...
5. **Easy continuous distribution constructors**: N (`normal`_), U (`uniform`_),
   Exp (`exponential`_), Gamma (`gamma`_), Beta (`beta`_), LogN 
   (`log-normal`_), X2 (`chi-squared`_), F (`f or fisher`_), Tri 
   (`triangular`_), T (`student's-t`_), Weib (`weibull`_). The location, scale, 
